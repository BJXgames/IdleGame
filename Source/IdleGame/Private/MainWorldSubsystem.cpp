...
FString UMainWorldSubsystem::FormatLargeNumber(double Number)
{
	...
	if (MagnitudeIndex == 0)
	{
		...
		FormattedString = FString::Printf(TEXT("%.0Lf"), FormattedNumber);
	}
	else
	{
		// Use "%.1Lf" for numbers with one decimal place
		FormattedString = FString::Printf(TEXT("%.1Lf %s"), FormattedNumber, *NumberSuffixes[MagnitudeIndex - 1].Suffix);
	}

	return FormattedString;
}

FString UMainWorldSubsystem::FormatNumber(double Number)
{
	const double Threshold = 1000.0L;
